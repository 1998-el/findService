...
$uploadDir = __DIR__ . './src/app/uploads';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($services) || empty($hourlyRate) || empty($address)) {
        ...
        try {
            ...
            if (!empty($profilePhoto)) {
                ...
                $fileName = 'worker_' . $userId . '_' . time() . '.jpg';
$filePath = $uploadDir . '/' . $fileName;
            ...
            $stmt = $pdo->prepare("INSERT INTO workers (user_id, profession, description, hourly_rate, availability) VALUES (?, ?, ?, ?, 'available')");
            $stmt->execute([$userId, $services, "Description du service", $hourlyRate]);
        ...
        <div class="form-section">
            ...
            <form action="worker_registration.php?user_id=<?php echo $userId; ?>" method="POST" enctype="multipart/form-data">
                ...
                <div class="input-group">
                    ...
                    <input type="text" id="address" name="address" required aria-label="Entrez votre adresse complète">
                </div>
